...
$$
P(Y_i = 1) = \pi_i = \frac{\mathbf{x}_i^T\boldsymbol{\beta}}{1+\mathbf{x}_i^T\boldsymbol{\beta}}
$$
...
As $D \gt \chi^2_{110}$ we see that there is overdispersion in the data. One can also divide the deviance, D, by the number of degrees of freedom, and if the number is close to 1 there is no or little to no sign of overdispersion. 
