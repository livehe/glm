...
Checking for overdispersion by doing a Chi-squared test. \textcolor{red}{why can we use chi-sq}. There are signs of overdispersion if the deviance, D, of a model is greater than the $\chi^2$ with $110$ degrees of freedom. 
```{r}
...
Potential sources for overdispersion can, in general, be unobserved heterogeneity and possitive correlation between individual binary observations of $Y$. For our data, there is quite possible a lot of heterogeneity, meaning there are many differences in our sample. When people are climbing mountains with different heights and different prominences, there is bound to be some differences in what different people are able to do. There could also be some correlation between the individual binary observations, as people tend to climb mountains in groups. If one person has to stop because of different reasons (maybe lack of fitness or simply an injury), there is a big chance the entire group quits or atleast some quit. 
...
The QAIC for model 4 is `r QIAC_4`. 
...
The QAIC is `r QIAC_5`. 

The model with lowest QAIC value is model 2, the model with 'intercept', 'height' and 'prominence' as parameters. \textcolor{red}{is phi also included here or just in the calc of QAIC?}
...
\textcolor{red}{hvorfor er å forklare coeffisientene under dette punktet lmao}
```{r}
coeffs <- coef(model2) 
coeffs
```
...
beta følger normalfordeling N(beta, invers av Fisher)
y=x0beta følger også normalfordeling x0beta, inv fisher * x0
lag konf int med dette
...
Transforming the confidence interval to the probability scale
...
The level 'BodoeGlimt' is chosen as the reference category, meaning that it is included in the intercept. \textcolor{red}{Making 15 levels?} 
...
As you can see, the correlation is $-0.055$, which is a medium negative correlation value on the scale from [-1,0]. This suggests that there is evidence of 
...
There could be some underdispersion since there is correlation between each pair of rows in the data frame. 
